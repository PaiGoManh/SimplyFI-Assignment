...
export ORDERER_ADMIN_TLS_SIGN_CERT=${PWD}/organizations/ordererOrganizations/harvest2home.com/orderers/orderer.harvest2home.com/tls/server.crt

export ORDERER_ADMIN_TLS_PRIVATE_KEY=${PWD}/organizations/ordererOrganizations/harvest2home.com/orderers/orderer.harvest2home.com/tls/server.key
...
peer channel fetch config ${PWD}/channel-artifacts/config_block.pb -o localhost:7050 --ordererTLSHostnameOverride orderer.harvest2home.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA
sleep 1
...
jq '.channel_group.groups.Application.groups.FarmerMSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.farmer.harvest2home.com","port": 7051}]},"version": "0"}}' config_copy.json >modified_config.json
...
peer channel update -f ${PWD}/channel-artifacts/config_update_in_envelope.pb -c $CHANNEL_NAME -o localhost:7050 --ordererTLSHostnameOverride orderer.harvest2home.com --tls --cafile $ORDERER_CA
sleep 1
...
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.harvest2home.com --channelID $CHANNEL_NAME --name Harvest2home --version 1.0  --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 2
...
echo "—---------------Join delivery-partner peer to the channel—-------------"

peer channel join -b ${PWD}/channel-artifacts/$CHANNEL_NAME.block
sleep 1
peer channel list

echo "—-------------delivery-partner anchor peer update—-----------"
...
jq '.channel_group.groups.Application.groups.DeliverypartnerMSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.delivery-partner.harvest2home.com","port": 9051}]},"version": "0"}}' config_copy.json >modified_config.json
...
echo "—---------------Approve chaincode in delivery-partner peer—-------------"
...
echo "—---------------Join consumer-association peer to the channel—-------------"

peer channel join -b ${PWD}/channel-artifacts/$CHANNEL_NAME.block
sleep 1
peer channel list

echo "—-------------consumer-association anchor peer update—-----------"
...
jq '.channel_group.groups.Application.groups.ConsumersAssociationMSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.consumer-association.harvest2home.com","port": 10051}]},"version": "0"}}' config_copy.json >modified_config.json
...
echo "—---------------Approve chaincode in consumer-association peer—-------------"
...
echo "—---------------Join quality-assurance-agency peer to the channel—-------------"

peer channel join -b ${PWD}/channel-artifacts/$CHANNEL_NAME.block
sleep 1
peer channel list

echo "—-------------quality-assurance-agency anchor peer update—-----------"
...
jq '.channel_group.groups.Application.groups.QualityAssuranceAgencyMSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.quality-assurance-agency.harvest2home.com","port": 12051}]},"version": "0"}}' config_copy.json >modified_config.json
...
echo "—---------------Approve chaincode in quality-assurance-agency peer—-------------"

peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.harvest2home.com --channelID $CHANNEL_NAME --name Harvest2home --version 1.0 --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 1

echo "—---------------Commit chaincode in quality-assurance-agency peer—-------------"

peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name Harvest2home --version 1.0 --sequence 1 --tls --cafile $ORDERER_CA --output json

peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.harvest2home.com --channelID $CHANNEL_NAME --name Harvest2home --version 1.0 --sequence 1 --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $FARMER_PEER_TLSROOTCERT --peerAddresses localhost:9051 --tlsRootCertFiles $DELIVERYPARTNER_PEER_TLSROOTCERT --peerAddresses localhost:10051 --tlsRootCertFiles $CONSUMERASSOCIATION_PEER_TLSROOTCERT --peerAddresses localhost:12051 --tlsRootCertFiles $QUALITYASSURANCE_PEER_TLSROOTCERT sleep 1

peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name Harvest2home --cafile $ORDERER_CA
